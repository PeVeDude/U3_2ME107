	...
	public function getDishes($dishes) {	
		...
		foreach ($dishArray as &$dish) {
			...
			if(!$dish == "") {
				...
				$dishNames 	= "<div class='".$i."' style='margin-bottom:7px;'>";
				$dishNames .= "<input type='checkbox' id='checkbox".$i."' name='dish' value='{$dishName[1]}'>";
				$dishNames .= "<i class='name'>{$dishName[0]}</i>";
				$dishNames .= "<div class='star".$i."' style='display:none; margin-top:10px;'></div>";
				$dishNames .= "</div><div id='commentDiv".$i."' style='display:none'>";
				$dishNames .= "<input type='text' id='comment".$i."' name='comment".$i."' placeholder='Comment..' disabled>";
				$dishNames .= "<i class='optional'> * Optional</i>";
				echo 		$dishNames;
				getPics($dishName[0]);
				echo 		"</div><br/>";
				$i++;
